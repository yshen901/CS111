...
for repeatedBlock in repeatedBlocks:
    for block in blocks:
        if block["block_num"] == repeatedBlock:
            corrupted = True
            if block["indirect_level"] == 0:
                print("DUPLICATE BLOCK " + str(block["block_num"]) + " IN INODE " + str(block["inode_num"]) + " AT OFFSET " + str(block["offset"]))
...
for inode in inodes:
    for i in range(0, len(inode.blocks)):
        ...
        if i == 12:
            type = "INDIRECT BLOCK"
        if i == 13:
            type = "DOUBLE INDIRECT BLOCK"
        if i == 14:
            type = "TRIPLE INDIRECT BLOCK"

        if inode.blocks[i] < 0 or inode.blocks[i] > superblock.num_blocks:
            print("INVALID " + type + " " + str(inode.blocks[i]) + " IN INODE " + str(inode.inode_num) + " AT OFFSET " + str(offset))
        if inode.blocks[i] in [1, 2, 3, 4, 5, 6, 7, 64]:
            print("RESERVED " + type + " " + str(inode.blocks[i]) + " IN INODE " + str(inode.inode_num) + " AT OFFSET " + str(offset))
...
for indirectBlock in indirectBlocks:
    ...
    if indirectBlock.block_num < 0 or indirectBlock.block_num > superblock.num_blocks:
        print("INVALID " + type + " " + str(indirectBlock.block_num) + " IN INODE " + str(inode) + " AT OFFSET " + str(offset))
    if indirectBlock.block_num in [0, 1, 2, 3, 4, 5, 6, 7, 64]:
        print("RESERVED " + type + " " + str(indirectBlock.block_num) + " IN INODE " + str(inode) + " AT OFFSET " + str(offset))
...
# CHECK DIRECTORY . AND .. INCONSISTENCIES
for directory in directories:
    name = directory.name
    parent = directory.parent_inode
    inode = directory.referenced_inode
    if name == "'.'":
        if parent != inode:
            corrupted = True
            print ("DIRECTORY INODE " + str(parent) + " NAME " + name + " LINK TO INODE " + str(inode) + " SHOULD BE " + st(parent))
    if name == "'..'":
        if parent == 2 and inode != 2:
            corrupted = True
            print ("DIRECTORY INODE " + str(parent) + " NAME " + name + " LINK TO INODE " + str(inode) + " SHOULD BE " + str(parent))
